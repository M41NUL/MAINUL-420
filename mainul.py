...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/M41NUL/MAINUL.Approval.txt.2/blob/main/mainul231.approval.txt"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/M41NUL/MAINUL.Approval.txt.2/blob/main/mainul231.approval.txt').text
			if basesplit in plr:
				key = basesplit
...
#				exit()
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR  MAINUL "%(Y,G,B,Y))
	...
	def old_9(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR MAINUL "%(Y,W,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[☠️ MAINUL ☠️] :\033[0;97m %s/%s-\033[0;92m[MAINUL-OK🔥:%s ]-\033[0;93m[MAINUL-CP 😿:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
